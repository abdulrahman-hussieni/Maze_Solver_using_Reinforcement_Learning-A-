...
class MazeEnv(gym.Env):
    ...
    def initialize_env(self, dimensions = 10, max_steps = 2000, walls_door = 2, player_start_pos = 'standard'):
        ...
        # GUI compatibility
        self.goal = self.objective_position
        self.state = self.player_marker_position
        self.init_pos = self.player_marker_start_pos
    ...
    def step(self, action):
        ...
        if(self.num_movements > self.max_steps):
            ...
            return [y_pos, x_pos, up_square, down_square, left_square, right_square], reward, done, {"num_steps": self.num_movements}
        else:
            return [y_pos, x_pos, up_square, down_square, left_square, right_square], reward, done, {"num_steps": self.num_movements}
    ...
    def reset(self):
        ...
        left_square = self.get_square_type(self.maze[y_pos][x_pos - 1])
        right_square = self.get_square_type(self.maze[y_pos][x_pos + 1])
        return [y_pos, x_pos, up_square, down_square, left_square, right_square], 0, False, {"num_steps": self.num_movements}
    ...
    def generate_maze_cells(self):

        H = self.maze.shape[0]
        W = self.maze.shape[1]

        self.maze_cells = np.zeros((H, W), dtype=np.uint8)

        for row in range(H):
            for col in range(W):

                # skip walls
                if self.maze[row][col] == self.wall_marker:
                    continue

                val = 0

                # WEST
                if col > 0 and self.maze[row][col - 1] != self.wall_marker:
                    val |= 0x1

                # NORTH
                if row > 0 and self.maze[row - 1][col] != self.wall_marker:
                    val |= 0x2

                # EAST
                if col < W - 1 and self.maze[row][col + 1] != self.wall_marker:
                    val |= 0x4

                # SOUTH
                if row < H - 1 and self.maze[row + 1][col] != self.wall_marker:
                    val |= 0x8

                self.maze_cells[row][col] = val
